...
class ReadAllView extends Component {
    ...
    fetchAllCollections = () => {
        ...
        xhr.open('get', '/browse/readAllCollections');
    ...
    onScroll = (e) => {
        if (this.state.finished === false && document.title === "Browse collections" && this.state.searching === false)
    ...
    loadAndAppendCollections = (loadAfter) => {
        ...
        if (this.state.finished === false) {
            ...
            const xhr = new XMLHttpRequest();
            xhr.open('post', '/browse/loadMoreCollections');
    ...
    onSearch = () => {
        ...
        if (this.state.searchQuery){
            ...
            const xhr = new XMLHttpRequest();
            xhr.open('post', '/browse/searchCollections');
            xhr.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
            xhr.responseType = 'json';
            xhr.addEventListener('load', () => {
                if (xhr.status === 200){
